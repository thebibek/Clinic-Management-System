<script>
    var app = angular.module('SalarySlipApp', []);
    app.controller('SalarySlipAppCtrl', function ($scope, $http, $window, $filter) {
        var config = {
            headers: {
        ...
        $scope.searchEmployeSalary = function () {
            ...
            $http.post('<?php echo base_url('app/filter/salary/slip'); ?>', $Data, config).then(function (response) {
                ...
                if (response.data.status === -1) {
                    swal("Network error,Please try again !!", {
                        ...
                        if (ok) {
                            ...
                            $window.location.href = '<?php echo base_url('app/salary/slip/print'); ?>';
