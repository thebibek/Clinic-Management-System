<script>
    var app = angular.module('ProfitAndLossApp', []);
    app.controller('ProfitAndLossAppCtrl', function($scope, $http, $window, $filter) {
        var config = {
            headers: {
        ...
        //initial value
        $scope.DebitBalance = '0.00';
        $scope.CreditBalance = '0.00';
        $scope.Profit = '0.00';
        ...
        function showProfitAndLoss() {
            ...
            $http.post('<?php echo base_url('api/provide/account/profit/loss'); ?>', $Data, config).then(function(response) {
                ...
                if(response.data.status === 1){
                    $scope.expenses = response.data.rs2;
                    $scope.incomes = response.data.rs1;
                    $scope.DebitBalance = response.data.DrBalance;
                    $scope.CreditBalance = response.data.CrBalance;
                    $scope.Profit = response.data.NetProfit;
                }
